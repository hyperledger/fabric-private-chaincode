...
do
    ...
    for protos in $(find "$FABRIC_PROTOS_DIR" -name '*.proto' -path "*/$i/*" -exec dirname {} \; | sort | uniq) ; do
        $PROTOC_CMD "$PROTOC_OPTS" --proto_path="${PROTOS_DIR}/google" --proto_path="$BUILD_DIR" --proto_path="$FABRIC_PROTOS_DIR" --nanopb_out=${FABRIC_BUILD_DIR} --nanopb_opt="-I${PROTOS_DIR} -f ${PROTOS_DIR}/fabric.options" "$protos"/*.proto
    done
done

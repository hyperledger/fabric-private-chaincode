...
help(){
   ...
   volumes or delete the chaincode images that have been created for the auctioncc
   options:
       --clean-slate:
           As part of the teardown, prune all unused docker volumes and delete auctioncc images.
       --help,-h:
...
if $CLEAN_SLATE; then
    ...
    image=$(go run ${DEMO_SCRIPT_DIR}/../../utils/fabric/get-fabric-container-name.src/get-fabric-container-name.go --cc-name auctioncc --peer-id peer0.org1.example.com --net-id dev_test --cc-version 1.0)
